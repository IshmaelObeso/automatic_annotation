...
def main():
    ...
    # create label and checkbox for using multithreading
    lbl_multiprocessing = ttk.Label(settings_tab, text='Enable Multiprocessing').grid(column=0, row=3)
    multiprocessing = BooleanVar(value=True)
    ent_multiprocessing = ttk.Checkbutton(settings_tab, variable=multiprocessing).grid(column=1, row=3, sticky='w')


    # create label and checkbox for generating triplets and statics
    lbl_generate_triplets_and_statics = ttk.Label(settings_tab, text='Generate Triplets and Statics').grid(column=0, row=4)
    ...
    def pass_user_input():
        ...
        multiprocessing_input = multiprocessing.get()
